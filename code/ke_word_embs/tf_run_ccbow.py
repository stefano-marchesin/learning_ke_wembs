...
flags.DEFINE_float("learning_rate", 0.025, "Learning rate used to optimize CBOW objective function.")
flags.DEFINE_float("alpha", 0.1, "Strength of synonymys loss function during learning.")
...
flags.DEFINE_string("model_name", "tf_ccbow_01_ohsu", "Model name.")
FLAGS = flags.FLAGS
...
def main(_):
	...
	if not os.path.exists(data_folder + '/words.json'):
		...
		# load required data
		print('load processed data required to train Constrained CBOW (CCBOW) model')
	...
	# for each word store the cfs of its synonyms
	print('get for each word the corpus frequencies of its synonyms')
	weights = {}
	# convert cfs to dict
	cfs = dict(cfs)
	for ix, synset in syns.items():
		# store word relative frequnecy 
		if synset:
			weights[ix] = [cfs[reverse_word_dict[syn]] for syn in synset]
		else:
			weights[ix] = []
	# convert syns and weights from dict of lists to list of lists - ordered by key
	syns = [synset for _, synset in sorted(zip(syns.keys(), syns.values()))]
	weights = [syn_weights for _, syn_weights in sorted(zip(weights.keys(), weights.values()))]
	# count the number of synonymy relations within corpus
	num_syns = sum([len(synset) for synset in syns])
	print('Total number of synonymy relations within {}: {}'.format(opts.corpus_name, num_syns))
	...
	with tf.Graph().as_default(), tf.Session() as sess:
		...
		# setup the model
		model = CCBOW(len(word_dict), syns, weights, opts)
