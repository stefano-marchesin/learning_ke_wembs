...
def run_quickumls_server(opts):
    matcher = QuickUMLS(
        ...
        accepted_semtypes = {semtype for semtype in opts.accepted_semtypes.split(',')},
        window=opts.window,
...
def parse_args():
    ...
    ap.add_argument(
        '-a', '--accepted_semtypes', default=','.join(constants.ACCEPTED_SEMTYPES), type=str,
        help='set of accepted semtypes')
    ap.add_argument(
